...
def ensure_canvas(coords, canvas_size: Tuple[int, int]):
    ...
    w_mask = np.any(np.logical_and(coords[..., 0] > 0, coords[..., 0] < w),
                    axis=1)
    h_mask = np.any(np.logical_and(coords[..., 1] > 0, coords[..., 1] < h),
                    axis=1)
...
def collate_fn_v2(
    ...
    if "rgb" in examples[0]:
        # multi-view images
        pixel_values = torch.stack(
            [example["rgb"].data for example in examples])
    ...
    pixel_values = torch.stack([example["depth"].data for example in examples])
    pixel_values = pixel_values.to(
        memory_format=torch.contiguous_format).float()
    ret_dict["depth"] = pixel_values

    pixel_values = torch.stack(
        [example["semantic_map"].data for example in examples])
    pixel_values = pixel_values.to(
        memory_format=torch.contiguous_format).float()
    ret_dict["semantic_map"] = pixel_values

    # captions: one real caption with one null caption
    input_ids_padded, captions = _tokenize_captions(examples, template,
                                                    tokenizer, is_train)
