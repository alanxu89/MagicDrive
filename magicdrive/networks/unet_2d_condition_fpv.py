...
from .blocks_video import (
    BasicMultiviewVideoTransformerBlock, )


class UNet2DConditionModelFPV(UNet2DConditionModel):
    r"""
                ...
                if crossview_attn_type == "basic":
                    _set_module(
                        ...
                        BasicMultiviewVideoTransformerBlock(
                            ...
                            upcast_attention=mod.attn1.upcast_attention,
                            norm_elementwise_affine=mod.
                            norm_elementwise_affine,
                            norm_type=mod.norm_type,
                            final_dropout=isinstance(mod.ff.net[-1],
                                                     nn.Dropout),
                    ...
                    raise TypeError(
                        f"Unknown attn type: {crossview_attn_type}")
    ...
    def trainable_module(self) -> Dict[str, nn.Module]:
        if self.trainable_state == "all":
            ...
            raise ValueError(
                f"Unknown trainable_state: {self.trainable_state}")
        ...
        for module in self.modules():
            if isinstance(module, (CrossAttnDownBlock2D, DownBlock2D,
                ...
                if flag is not None and not flag[mod_idx]:
                    logging.debug(
                        f"[UNet2DConditionModelFPV] "
                        f"gradient_checkpointing skip [{module.__class__}]")
                    continue
                logging.debug(
                    f"[UNet2DConditionModelFPV] set "
    ...
    def from_unet_2d_condition(
        ...
        **kwargs,
        ...
        r"""
        Instantiate FPV unet class from UNet2DConditionModel.
    ...
    def forward(
        ...
        if any(s % default_overall_up_factor != 0 for s in sample.shape[-2:]):
            if self._WARN_ONCE == 0:
                logging.warning(
                    "[UNet2DConditionModelFPV] Forward upsample size to force interpolation output size."
                )
                self._WARN_ONCE = 1
        ...
        for downsample_block in self.down_blocks:
            ...
            if hasattr(downsample_block, "has_cross_attention"
                ...
                sample, res_samples = downsample_block(
                    ...
                    cross_attention_kwargs=copy.deepcopy(
